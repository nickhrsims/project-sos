...
template <uint8_t index> SDL_Window* get() { return __data__::sdl_window<index>; }
...
template <uint8_t index> void create(const config& p_config) {
    ...
    __data__::sdl_window<index> = SDL_CreateWindow(
        p_config.title.c_str(), p_config.x, p_config.y, p_config.w, p_config.h, 0);
