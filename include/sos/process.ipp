...
void sos::process::start(Fn p_processing_function, uint8_t p_frames_per_second) {
    ...
    // FPS in Milliseconds
    // == 1 (frame) / ratio (seconds) * 1000 (convert to ms)
    float const fps_ms = 1.0f / p_frames_per_second * 1000.0f;
    ...
    while (active) {
        ...
        milliseconds frame_delay = std::abs(std::floor(fps_ms - frame_duration));
